        ...
        <div style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,overflow:"hidden"}}>
          <div style={{padding:"12px 16px",borderBottom:`1px solid ${S.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <button onClick={()=>{
                const cols=["Month","Current Year","Previous Year","Current","Previous","Difference","Diff %"];
              ...
              <div style={{overflowX:"auto",maxHeight:500,overflowY:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead><tr>
                    ...
                    <th style={{...TH,color:S.warn}}>Δ Royal</th>
                    <th style={{...TH,color:S.warn}}>Δ First</th>
                    <th style={{...TH,color:S.warn}}>Δ Premium</th>
                    <th style={{...TH,color:S.warn}}>Δ Total</th>
                  </tr></thead>
                  <tbody>
                    {pendel.length===0&&<tr><td colSpan={14} style={{padding:24,textAlign:"center",color:S.muted}}>No data</td></tr>}
                    {pendel.map((r,i)=>(
                      <tr key={i} style={{borderBottom:`1px solid ${S.border}`,background:i%2===0?"transparent":"rgba(255,255,255,0.018)"}}>
                        <td style={{...TDL,color:S.accent2,fontWeight:600}}>{r.StartDate}</td>
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{background:S.side,borderBottom:`1px solid ${S.border}`,padding:"12px 18px",flexShrink:0}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-end",flexWrap:"wrap"}}>
          ...
          <div>
            <label style={{fontSize:10,color:S.muted,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>Status</label>
          ...
          <button onClick={()=>{const p=buildParams(f,1);setPage(1);load(p,1);}} style={{padding:"6px 16px",background:S.accent,border:"none",borderRadius:6,color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",alignSelf:"flex-end"}}>Apply</button>
        </div>
        ...
        <div style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,overflow:"hidden"}}>
          <div style={{padding:"11px 14px",borderBottom:`1px solid ${S.border}`,display:"flex",gap:10,alignItems:"center"}}>
            <div style={{fontSize:14,fontWeight:700,color:S.text,flex:1}}>
              ...
              <span style={{fontSize:11,color:S.muted,fontWeight:400,marginLeft:8}}>({fmtN(filtered.length)} rows)</span>
            </div>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search booking ID…" style={{...inpS,width:180}}/>
            <button onClick={()=>{
              const cols=["BookingID","StatusCode","BookingDate","DepartureDate","ReturnDate","TravelType","PAX","SalesBooking","PurchaseCalculation","PurchaseObligation","Margin","Commission","MarginIncludingCommission","PaidPassed","Outstanding"];
              const csv=[cols.join(","),...filtered.map(r=>cols.map(c=>String(r[c]??"")))].join("\n");
                  ...
                  <thead style={{position:"sticky",top:0,zIndex:5,background:S.bg}}>
                    <tr>
                      {TABLE_COLS.map(([h,a],i)=>(
                        <th key={i} style={{padding:"8px 11px",textAlign:a,color:S.muted,fontWeight:700,fontSize:10,textTransform:"uppercase",letterSpacing:"0.05em",whiteSpace:"nowrap",borderBottom:`1px solid ${S.border}`}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
...
function SettingsTab({token,session,onLogout,theme,setTheme}){
  ...
  useEffect(()=>{
    api("/api/dashboard/users",{},token).then(d=>setUsers(Array.isArray(d)?d:[])).catch(()=>{});
  ...
  async function addUser(e){
    ...
    try{
      ...
      setUsers(u=>[...u,d]);setNewUser({username:"",password:"",role:"viewer"});setUserMsg("User created.");
    }catch(e){setUserMsg("Error: "+e.message);}finally{setBusy(false);}
  }
  async function deleteUser(id){
    if(!confirm("Delete this user?"))return;
    try{await fetch(`${BASE}/api/dashboard/users/${id}`,{method:"DELETE",headers:{Authorization:`Bearer ${token}`}});setUsers(u=>u.filter(x=>x.id!==id));}
    catch(e){setUserMsg("Error: "+e.message);}
  }
  async function updateUserRole(id,role){
    try{const r=await fetch(`${BASE}/api/dashboard/users/${id}`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({role})});const d=await r.json();setUsers(u=>u.map(x=>x.id===id?d:x));}
    catch(e){setUserMsg("Error: "+e.message);}
  }
  ...
  const inp=(val,onChange,type="text",ph="")=>(<input type={type} value={val} onChange={e=>onChange(e.target.value)} placeholder={ph} style={{background:S.bg,border:`1px solid ${S.border2}`,borderRadius:7,padding:"8px 11px",color:S.text,fontSize:13,width:"100%",boxSizing:"border-box"}}/>);
  ...
  const card=(children,style={})=>(<div style={{background:S.card,border:`1px solid ${S.border}`,borderRadius:12,padding:"18px 20px",marginBottom:16,...style}}>{children}</div>);
  const sTabBtn=(id,label)=>(<button onClick={()=>setTab(id)} style={{padding:"8px 16px",borderRadius:8,fontSize:12,cursor:"pointer",border:`1px solid ${tab===id?S.accent:S.border2}`,background:tab===id?"rgba(59,130,246,0.15)":"transparent",color:tab===id?S.accent2:S.muted,fontWeight:600}}>{label}</button>);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{background:S.side,borderBottom:`1px solid ${S.border}`,padding:"10px 18px",display:"flex",gap:8,flexShrink:0,flexWrap:"wrap"}}>
        {sTabBtn("users","User Management")}{sTabBtn("theme","Theme")}{sTabBtn("api","API Status")}{sTabBtn("ai","AI Prompts")}{sTabBtn("alerts","Email Alerts")}
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"18px 24px",maxWidth:720}}>
        ...
        {tab==="users"&&(<>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {users.map(u=>(
                <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:S.bg,borderRadius:8,border:`1px solid ${S.border}`}}>
                  ...
                  <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:S.text}}>{u.username}</div><div style={{fontSize:11,color:S.muted}}>ID: {u.id}</div></div>
                  <select value={u.role} onChange={e=>updateUserRole(u.id,e.target.value)} style={{background:S.bg,border:`1px solid ${S.border2}`,borderRadius:6,padding:"4px 8px",color:S.text,fontSize:12}}><option value="viewer">Viewer</option><option value="admin">Admin</option></select>
                  ...
                  {u.id!==session?.id&&u.username!==session?.username&&(<button onClick={()=>deleteUser(u.id)} style={{background:"rgba(239,68,68,0.1)",border:`1px solid rgba(239,68,68,0.3)`,borderRadius:6,padding:"4px 10px",color:S.danger,fontSize:11,cursor:"pointer",fontWeight:600}}>Delete</button>)}
                </div>
              ))}
          ...
          {card(<>
            ...
            <form onSubmit={addUser} style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div>{slab("Role")}<select value={newUser.role} onChange={e=>setNewUser({...newUser,role:e.target.value})} style={{background:S.bg,border:`1px solid ${S.border2}`,borderRadius:7,padding:"8px 11px",color:S.text,fontSize:13,width:"100%"}}><option value="viewer">Viewer</option><option value="admin">Admin</option></select></div>
              <button type="submit" disabled={busy||!newUser.username||!newUser.password} style={{padding:"9px",background:S.accent,border:"none",borderRadius:8,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",opacity:busy?0.7:1}}>{busy?"Creating…":"Create User"}</button>
            </form>
          </>)}
          {card(<>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:S.text}}>{session?.username}</div><div style={{fontSize:12,color:S.muted}}>Role: {session?.role||"viewer"}</div></div>
        ...
        {tab==="theme"&&(<>
          ...
          {card(<>
            ...
            <div style={{display:"flex",gap:12}}>
              {[["dark","Dark Mode","#0f172a","#1e293b"],["light","Light Mode","#f8fafc","#e2e8f0"]].map(([id,label,bg,c])=>(
                <div key={id} onClick={()=>setTheme(id)} style={{flex:1,padding:16,borderRadius:10,border:`2px solid ${theme===id?S.accent:S.border}`,cursor:"pointer",background:bg}}>
                  <div style={{width:"100%",height:40,borderRadius:6,background:c,marginBottom:8}}></div>
          ...
          {card(<>
            ...
            {["CustomerOverview","ST_Bookings","solmar_bus_deck_choice","BUStrips","FeederOverview","solmar.MarginOverview"].map((name,i,arr)=>(
              <div key={name} style={{display:"flex",alignItems:"center",padding:"9px 0",borderBottom:i<arr.length-1?`1px solid ${S.border}`:"none"}}>
                <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:S.text,fontFamily:"monospace"}}>{name}</div></div>
        ...
        {tab==="api"&&(<>
          <div style={{fontSize:15,fontWeight:700,color:S.text,marginBottom:14}}>API Status</div>
          {card(<>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {API_ENDPOINTS.map(ep=>{
                ...
                return(
                  <div key={ep.name} style={{display:"flex",alignItems:"center",padding:"10px 14px",background:S.bg,borderRadius:8,border:`1px solid ${S.border}`,gap:10}}>
                    ...
                    <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:S.text}}>{ep.name}</div><div style={{fontSize:11,color:S.muted,fontFamily:"monospace"}}>{ep.path}</div></div>
        ...
        {tab==="ai"&&(<>
          ...
          {card(<>
            ...
            <textarea value={settings.aiPrompt||""} onChange={e=>setSettings({...settings,aiPrompt:e.target.value})} rows={8} placeholder="We are a Belgian travel company. Revenue in EUR. Fiscal year Dec–Nov…" style={{width:"100%",background:S.bg,border:`1px solid ${S.border2}`,borderRadius:8,padding:"10px 12px",color:S.text,fontSize:13,resize:"vertical",boxSizing:"border-box"}}/>
        ...
        {tab==="alerts"&&(<>
          ...
          {card(<>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              ...
              <div style={{position:"relative",width:42,height:22,cursor:"pointer"}} onClick={()=>setSettings({...settings,emailAlerts:{...settings.emailAlerts,enabled:!settings.emailAlerts?.enabled}})}>
                <div style={{width:42,height:22,borderRadius:11,background:settings.emailAlerts?.enabled?S.accent:S.border}}/>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div>{slab("Recipients")}{inp(settings.emailAlerts?.recipients||"",v=>setSettings({...settings,emailAlerts:{...settings.emailAlerts,recipients:v}}),"text","email@example.com")}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div>{slab("Revenue Drop %")}<input type="number" value={settings.emailAlerts?.revenueDropThreshold||10} onChange={e=>setSettings({...settings,emailAlerts:{...settings.emailAlerts,revenueDropThreshold:+e.target.value}})} style={{background:S.bg,border:`1px solid ${S.border2}`,borderRadius:7,padding:"8px 11px",color:S.text,fontSize:13,width:"100%",boxSizing:"border-box"}}/></div>
                <div>{slab("Booking Spike %")}<input type="number" value={settings.emailAlerts?.bookingSpikethreshold||20} onChange={e=>setSettings({...settings,emailAlerts:{...settings.emailAlerts,bookingSpikethreshold:+e.target.value}})} style={{background:S.bg,border:`1px solid ${S.border2}`,borderRadius:7,padding:"8px 11px",color:S.text,fontSize:13,width:"100%",boxSizing:"border-box"}}/></div>
              </div>
...
export default function App(){
  ...
  const NAV=[
    {id:"overview",l:"Overview",ic:<svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>},
    {id:"bus",l:"Bus Occupancy",ic:<svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 6v6M3 16V8a4 4 0 014-4h10a4 4 0 014 4v8m0 0H3m18 0v4H3v-4"/><circle cx="7.5" cy="16.5" r="1.5"/><circle cx="16.5" cy="16.5" r="1.5"/></svg>},
    {id:"purchase",l:"Purchase Obligations",ic:<svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>},
    {id:"settings",l:"Settings",ic:<svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 15a3 3 0 100-6 3 3 0 000 6z"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>},
  ];
