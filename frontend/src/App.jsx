...
function UserTable({users,loading,session,loadUsers,openEdit,deleteUser,updateUserRole}){
...
function SettingsTab({token,session,onLogout}){
  ...
  const sTabBtn=(id,label,icon)=>(
    <button onClick={()=>setStab(id)} style={{padding:"8px 16px",borderRadius:7,fontSize:12,cursor:"pointer",border:`1.5px solid ${stab===id?S.accent:S.border2}`,background:stab===id?S.accentLight:"transparent",color:stab===id?S.accent:S.textLight,fontWeight:600,display:"flex",alignItems:"center",gap:5}}>
      {icon&&icon}{label}
    </button>
  );
      ...
      <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
        {stab==="users"&&(
          <div style={{maxWidth:1000}}>
            ...
            {showAdd&&(
              <div style={{background:S.card,border:`1.5px solid ${S.accent}33`,borderRadius:12,padding:20,marginBottom:20}}>
                ...
                <form onSubmit={addUser} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
                  {[["Full Name","name","text","Full name"],["Username","username","text","username"],["Email","email","email","email@example.com"]].map(([label,field,type,ph])=>(
                    <div key={field}>
                  ...
                  <div>
                    <label style={{fontSize:10,fontWeight:700,color:S.muted,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.05em"}}>Password</label>
                    <div style={{position:"relative"}}>
                      <input type={showNewUserPw?"text":"password"} value={newUser.password} onChange={e=>setNewUser({...newUser,password:e.target.value})} placeholder="Password"
                        style={{width:"100%",padding:"8px 36px 8px 10px",border:`1.5px solid ${S.border2}`,borderRadius:7,background:S.bg,color:S.text,fontSize:12,outline:"none",boxSizing:"border-box"}}/>
                      <button type="button" onClick={()=>setShowNewUserPw(p=>!p)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:S.muted2,display:"flex",alignItems:"center",padding:2}}>
                        {showNewUserPw?<EyeOff size={13}/>:<Eye size={13}/>}
                      </button>
                    </div>
                  </div>
                  <div>
            ...
            <UserTable users={users} loading={loading} session={session} loadUsers={loadUsers} openEdit={openEdit} deleteUser={deleteUser} updateUserRole={updateUserRole}/>
          </div>
        )}
        {stab==="api"&&(
